...
function NavButton({
  ...
  return (
    <button
      onClick={onClick}
      aria-label={label}
      className={`w-[44px] h-[44px] rounded-[22px] flex items-center justify-center transition-all ${
        isActive
          ? "bg-accent/25 border border-accent/50 text-accent"
          : "bg-white/[0.05] border border-white/[0.08] text-text-secondary hover:text-text-primary hover:bg-white/[0.08]"
      }`}
    >
      <Icon size={20} />
    </button>
